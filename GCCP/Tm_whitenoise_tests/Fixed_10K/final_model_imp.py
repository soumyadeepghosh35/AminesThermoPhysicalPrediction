...
def build_model_with_bounded_params(X, Y, kern, low, high, \
    ...
    if kern == "RQ":
        ...
        kernel_.variance = var
    if useWhite == True:
        variance_10K = 0.002371207000097934
        final_kernel = kernel_+gpflow.kernels.White(variance=variance_10K)
    else:
    ...
    if useWhite == True:
        gpflow.utilities.set_trainable(model_.kernel.kernels[1].variance,False)
    model = model_
    return model
