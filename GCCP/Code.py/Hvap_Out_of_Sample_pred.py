...
bars1 = ax.bar(x - width/2, Hvap_F_gc, width, label='JR GC', color='red', hatch=hatches[0])
bars2 = ax.bar(x + width/2, Hvap_F_exp, width, label='Exp.', color='blue', hatch=hatches[1])
bars3 = ax.bar(x + 3*width/2, Hvap_F_gcgp, width, label='GCGP', color='green', yerr=Hvap_F_gcgp_error , capsize=5, hatch=hatches[2])


ax.set_ylabel('$\Delta H_{vap}$ kJ/mol', size=16)
...
plt.ylim(0, 80)
ax.legend(ncol=3, fontsize=14, loc='upper center', bbox_to_anchor=(0.5, 1.15))
