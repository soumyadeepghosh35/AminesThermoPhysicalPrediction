...
random_seeds_ = [random.randint(0, 1000000) for _ in range(10)]
random_seeds = [42] + random_seeds_
print(random_seeds)
...
num_output_cols = 10
train_test_split_res = np.zeros([len(random_seeds), num_output_cols])

for i, seed_ in enumerate(random_seeds):
    np.random.seed(seed_)
    ...
    count_CI = np.array(count_CI).flatten()

    # Save the model and other info summary to a CSV file
    model_file_name = str(save_path +f'/{seed_}_model_summary.txt')
    with open(model_file_name, 'w') as file:
        ...
        file.write("\n Number and Fraction outside 95CI: " + str(count_CI))
    
    perf_check_test = evaluate_output(Y_Test_plt, Y_Test_Pred_plt)
    perf_check_train = evaluate_output(Y_Train_plt, Y_Train_Pred_plt)

    seed_i_data = [seed_, best_lml, perf_check_test[1], perf_check_train[1], perf_check_test[2], perf_check_train[2], \
    perf_check_test[3], perf_check_train[3], perf_check_test[0], perf_check_train[0]]

    train_test_split_res[i, :] = seed_i_data

dir_root = "Final_Results/" + code
os.makedirs(dir_root, exist_ok=True)

data_cols = ['seed', 'LML', 'MAPE_tst', 'MAPE_trn', 'MAE_tst', 'MAE_trn', 'RMSE_tst', 'RMSE_trn', 'R2_tst', 'R2_trn', ]

train_test_split_res_df = pd.DataFrame(train_test_split_res, columns=data_cols)
cols_to_format = train_test_split_res_df.columns[1:]
train_test_split_res_df[cols_to_format] = train_test_split_res_df[cols_to_format].apply(pd.to_numeric, errors='coerce')
train_test_split_res_df[cols_to_format] = train_test_split_res_df[cols_to_format].round(2)

train_test_split_res_df.to_csv(dir_root+f'/{code}_train_test_split_res.csv', index=False)
